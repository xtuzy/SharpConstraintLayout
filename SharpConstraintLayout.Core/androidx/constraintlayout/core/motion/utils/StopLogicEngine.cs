...
namespace androidx.constraintlayout.core.motion.utils
{
	...
	/// @suppress
	/// </summary>
	public class StopLogicEngine : StopEngine
	{
		...
		/// </summary>
		/// <param name="desc"> Description to pre append </param>
		/// <param name="time"> Time during animation </param>
		/// <returns> string useful for debugging the state of the StopLogic </returns>
		...
		public virtual float getVelocity(float x)
		{
		...
		public virtual void config(float currentPos, float destination, float currentVelocity, float maxTime, float maxAcceleration, float maxVelocity)
		{
			mDone = false;
		...
		public virtual float getInterpolation(float v)
		{
			float y = calcY(v);
			mLastPosition = v;
		...
		private void setup(float velocity, float distance, float maxAcceleration, float maxVelocity, float maxTime)
		{
			...
			{ // backward
				...
				peak_v = (float) Math.Sqrt(maxAcceleration * totalDistance);
			...
			{ // we cannot make it hit the breaks.
			...
			{ // close enough maintain v then break
			...
			peak_v = (float) Math.Sqrt(maxAcceleration * distance + velocity * velocity / 2);
			this.mStage1Duration = (peak_v - velocity) / maxAcceleration;
			this.mStage2Duration = peak_v / maxAcceleration;
			if (peak_v < maxVelocity)
			{ // accelerate then decelerate
			...
			this.mStage1Duration = (maxVelocity - velocity) / maxAcceleration;
			this.mStage3Duration = maxVelocity / maxAcceleration;
			float tempaccDist = (velocity + maxVelocity) * this.mStage1Duration / 2;
			float tempdecDist = (maxVelocity * this.mStage3Duration) / 2;

			this.mStage2Duration = (distance - tempaccDist - tempdecDist) / maxVelocity;
			this.mStage1EndPosition = tempaccDist;
			this.mStage2EndPosition = (distance - tempdecDist);
			this.mStage3EndPosition = distance;
		}
	}
